...
public class WordSearch {
     ...
     public boolean exist(char[][] board, String word) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if(board[i][j] == first){
                   return  backtracking(board, visited, i, j, word, 0)  ;
                }
            }
        }
    ...
    private boolean backtracking(char[][] board , boolean visited[][] , int rowStart , int columnStart , String word ,int currnetCharIndex){
